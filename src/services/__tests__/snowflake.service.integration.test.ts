...
describe('SnowflakeUtils - Testes de Integração', () => {
  ...
  describe('Ordenação de IDs Snowflake', () => {
    it('deve gerar IDs que podem ser ordenados cronologicamente', () => {
      ...
      const ids = [
        SnowflakeUtils.fromTimestamp({ timestamp: timestamps[2], epoch: testEpoch }),
        SnowflakeUtils.fromTimestamp({ timestamp: timestamps[0], epoch: testEpoch }),
        SnowflakeUtils.fromTimestamp({ timestamp: timestamps[4], epoch: testEpoch }),
        SnowflakeUtils.fromTimestamp({ timestamp: timestamps[1], epoch: testEpoch }),
        SnowflakeUtils.fromTimestamp({ timestamp: timestamps[3], epoch: testEpoch })
      ];
      ...
      for (let i = 1; i < sortedTimestamps.length; i++) {
        expect(sortedTimestamps[i].getTime()).toBeGreaterThan(sortedTimestamps[i-1].getTime());
      }
  ...
  describe('Validação e comparação de IDs', () => {
    it('deve validar e comparar IDs corretamente', () => {
      ...
      const id1 = SnowflakeUtils.fromTimestamp({ timestamp: timestamp1, epoch: testEpoch });
      const id2 = SnowflakeUtils.fromTimestamp({ timestamp: timestamp2, epoch: testEpoch });
      
      // Valida os IDs
      expect(SnowflakeUtils.isValidSnowflake({ snowflakeId: id1.toString() })).toBe(true);
      expect(SnowflakeUtils.isValidSnowflake({ snowflakeId: id2.toString() })).toBe(true);
      
      // Compara os IDs - id2 deve ser maior (mais recente) que id1
      const comparisonResult = SnowflakeUtils.compare({ first: id2, second: id1 });
      ...
      const extractedTimestamp1 = SnowflakeUtils.getTimestamp({ snowflakeId: id1, epoch: testEpoch });
      const extractedTimestamp2 = SnowflakeUtils.getTimestamp({ snowflakeId: id2, epoch: testEpoch });
      
      expect(extractedTimestamp2.getTime()).toBeGreaterThan(extractedTimestamp1.getTime());
    });
  });
