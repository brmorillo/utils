...
describe('UUIDUtils - Testes de Benchmark', () => {
  ...
  describe('uuidV1Generate', () => {
    it('deve gerar 100.000 UUIDs v1 em tempo razoável', () => {
      ...
      console.log(`Tempo para gerar ${count} UUIDs v1: ${executionTime.toFixed(2)}ms`);
  ...
  describe('uuidV4Generate', () => {
    it('deve gerar 100.000 UUIDs v4 em tempo razoável', () => {
      ...
      console.log(`Tempo para gerar ${count} UUIDs v4: ${executionTime.toFixed(2)}ms`);
  ...
  describe('uuidV5Generate', () => {
    it('deve gerar 100.000 UUIDs v5 em tempo razoável', () => {
      ...
      const executionTime = measureExecutionTime(() => {
        for (let i = 0; i < count; i++) {
          ...
          uuids.push(UUIDUtils.uuidV5Generate({ 
            namespace, 
            name: `test-${i}` 
          }));
        }
      ...
      console.log(`Tempo para gerar ${count} UUIDs v5: ${executionTime.toFixed(2)}ms`);
    ...
    it('deve gerar 100.000 UUIDs v5 com namespace automático em tempo razoável', () => {
      ...
      console.log(`Tempo para gerar ${count} UUIDs v5 com namespace automático: ${executionTime.toFixed(2)}ms`);
  ...
  describe('uuidIsValid', () => {
    it('deve validar 100.000 UUIDs válidos em tempo razoável', () => {
      ...
      console.log(`Tempo para validar ${count} UUIDs válidos: ${executionTime.toFixed(2)}ms`);
    ...
    it('deve validar 100.000 strings inválidas em tempo razoável', () => {
      ...
      console.log(`Tempo para validar ${count} strings inválidas: ${executionTime.toFixed(2)}ms`);
  ...
  describe('Comparação de desempenho', () => {
    it('deve comparar o desempenho de geração entre diferentes versões de UUID', () => {
      ...
      console.log(`- UUID v1: ${v1Time.toFixed(2)}ms (${(v1Time / count).toFixed(5)}ms por UUID)`);
      console.log(`- UUID v4: ${v4Time.toFixed(2)}ms (${(v4Time / count).toFixed(5)}ms por UUID)`);
      console.log(`- UUID v5: ${v5Time.toFixed(2)}ms (${(v5Time / count).toFixed(5)}ms por UUID)`);
