...
describe('StringUtils - Testes de Integração', () => {
  describe('Cenários de uso real', () => {
    it('deve formatar um nome de usuário para exibição', () => {
      ...
      // Verificações
      expect(kebabCased).toBe('john-doe');
    ...
    it('deve processar um template de email com dados do usuário', () => {
      ...
      const emailTemplate = 'Olá, {name}! Sua conta foi criada com sucesso. Seu nome de usuário é {username}.';
      const userData = {
        name: 'Maria Silva',
      ...
      // Verificações
      expect(processedEmail).toBe('Olá, Maria Silva! Sua conta foi criada com sucesso. Seu nome de usuário é maria_silva_2023.');
    ...
    it('deve processar um slug para URL a partir de um título de artigo', () => {
      ...
      const articleTitle = 'Como Criar Testes Eficientes em JavaScript: Um Guia Completo';
      ...
      // Verificações
      expect(slug).toBe('como-criar-testes-eficientes-em-javascript-um-guia-completo');
      expect(truncatedSlug.length).toBeLessThanOrEqual(50);
    });
  });

  describe('Operações encadeadas', () => {
    it('deve realizar uma série de transformações em uma string', () => {
      // String inicial
      ...
      // Verificações
      expect(snakeCase).toBe('this_is_a_test_string_with_underscores_and_hyphens');
      expect(camelCase).toBe('thisIsATestStringWithUnderscoresAndHyphens');
      expect(kebabCase).toBe('this-is-a-test-string-with-underscores-and-hyphens');
      expect(isPalindrome).toBe(false);
      expect(reversed).toBe('snehpyh-dna-serocsrednu-htiw-gnirts-tset-a-si-siht');
      expect(isReversedPalindrome).toBe(true);
    });

    it('deve processar um texto para análise de conteúdo', () => {
      // Texto para análise
      const text = 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Ipsum dolor sit amet, consectetur.';
      ...
      expect(processedText).toBe('Lorem [palavra repetida] dolor sit amet, consectetur adipiscing elit. Ipsum dolor sit amet, consectetur.');
      expect(summary).toBe('Lorem [palavra repetida] dolor sit amet, consectetur...');
    });
  });
  ...
  describe('Validação e formatação de dados', () => {
    it('deve validar e formatar um nome completo', () => {
      ...
      const formattedFirstName = StringUtils.toTitleCase({ input: cleanFirstName });
      const formattedLastName = StringUtils.toTitleCase({ input: cleanLastName });
    ...
    it('deve validar e formatar um código de produto', () => {
      ...
      const formattedCategory = StringUtils.toSnakeCase({ input: productCategory });
      
      // 2. Formatar a variante em camelCase
      const formattedVariant = StringUtils.toCamelCase({ input: productVariant });
