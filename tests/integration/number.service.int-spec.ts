...
describe('NumberUtils - Testes de Integração', () => {
  describe('Formatação de valores monetários', () => {
    it('deve formatar valores monetários com precisão', () => {
      ...
      const precoFinal = NumberUtils.roundToDecimals({ value: precoComDesconto });
  ...
  describe('Validação e normalização de entradas', () => {
    it('deve validar e normalizar entradas numéricas', () => {
      ...
      for (const entrada of entradas) {
        ...
        const valorArredondado = NumberUtils.roundToNearest({ value: valorLimitado });
  ...
  describe('Cálculos matemáticos em cadeia', () => {
    it('deve realizar uma série de operações matemáticas', () => {
      ...
      const numeroAleatorio = NumberUtils.randomFloatInRange({ min: 1, max: 10 });
      ...
      const numeroInteiro = NumberUtils.removeDecimalPlaces({ value: numeroArredondado });
      ...
      if (fatorial > 0) {
        const fatorialEsperado = [1, 2, 6, 24, 120, 720, 5040, 40320, 362880, 3628800];
  ...
  describe('Geração de sequências numéricas', () => {
    it('deve gerar e processar uma sequência de números', () => {
      ...
      expect(resultados.maiorNumero).toBeGreaterThanOrEqual(resultados.menorNumero);
