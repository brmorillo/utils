...
expect(NumberUtils.roundToDecimals({ value: 3.14159, decimals: 3 })).toBe(3.142);
expect(NumberUtils.roundToDecimals({ value: 3.14159, decimals: 0 })).toBe(3);
    });
...
expect(NumberUtils.addDecimalPlaces({ value: 10.5, decimalPlaces: 3 })).toBe('10.500');
expect(NumberUtils.addDecimalPlaces({ value: 10, decimalPlaces: 2 })).toBe('10.00');
    });

    it('deve normalizar -0 para 0', () => {
expect(NumberUtils.addDecimalPlaces({ value: -0, decimalPlaces: 2 })).toBe('0.00');
    });
...
const result = NumberUtils.randomFloatInRange({ min: 1, max: 10, decimals: 3 });
