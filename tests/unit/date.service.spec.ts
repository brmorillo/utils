...
expect(Math.abs(localNow.toMillis() - systemNow.toMillis())).toBeLessThan(100);
...
// Verifica se a diferença é menor que 100ms
expect(Math.abs(utcNow.toMillis() - systemUtcNow.toMillis())).toBeLessThan(100);
...
const interval = DateUtils.createInterval({ startDate: start, endDate: end });
