...
> Note: Unlike most utilities in this library, `CryptUtils` methods use positional arguments (not a single destructured object).
...
const { encryptedData, iv } = CryptUtils.aesEncrypt('Hello, World!', secretKey);
...
const cipher = CryptUtils.rsaEncrypt('Secret', publicKey);
console.log(CryptUtils.rsaDecrypt(cipher, privateKey)); // "Secret"
```
...
```javascript
const secretKey = '12345678901234567890123456789012';
const { encryptedData, iv } = CryptUtils.aesEncrypt({ name: 'Alice' }, secretKey);
...
const encrypted = CryptUtils.chacha20Encrypt('Hello', key, nonce);
